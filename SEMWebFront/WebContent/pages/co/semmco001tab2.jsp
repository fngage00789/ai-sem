...
<h:panelGrid columnClasses="gridContent" width="100%">
	<h:panelGrid width="95%">
					<rich:panel id="pnlContractTot" rendered="#{semmco001Bean.renderedDataTab1 and semmco001Bean.renderSendTotForAis}">
						<f:facet name="header">
						...
						<h:panelGroup>
							...
							<h:selectOneRadio id="rbtTotSendFlag" value="#{semmco001tab1Bean.contract.totSendFlag}"  styleClass="ms7" rendered="true" 
							onclick="changeSend(this.value);" disabled="#{not semmco001tab1Bean.renderedEditTot}">
							...
							<h:graphicImage value="images/icon_required.gif" id="remarkRq" style="#{semmco001tab1Bean.contract.totSendFlag eq 'Y'?'display:none':''}"/>
							<rich:spacer width="5"></rich:spacer>
							<h:outputText value="#{jspMsg['label.totRemarkNotSend']}" styleClass="ms7" style="#{semmco001tab1Bean.contract.totSendFlag eq 'Y'?'display:none':''}" id="remarkLabel"/>
                			</td>
                			<td align="left" width="70%">
                			 <h:inputTextarea id="txtTotRemarkNotSend" value="#{semmco001tab1Bean.contract.totRemarkNotSend}" 
                			 disabled="#{not semmco001tab1Bean.renderedEditDuty}"
                			 rows="3" cols="100" style="#{semmco001tab1Bean.contract.totSendFlag eq 'Y'?'display:none':''}"/>
                			</td>
	                		</tr>
							</table>
						</h:panelGroup>
	...
	<h:panelGrid id="pnlTab2" width="95%">
						...
						<tr>     
							...
							<td>
								<a4j:commandButton id="btnUploadPicture"
									...
									<a4j:actionparam name="module" value="CO"/>
									<a4j:actionparam name="contractNo" value="#{semmco001Bean.contractNoParam}"/>
									...
									<tr>     
										...
										<td width="40%">
											<h:selectOneMenu id="ddlContractDocType" value="#{semeco001Bean.contractDocType}">
												<f:selectItems value="#{semmco001tab2Bean.contractDocType}" />	 			
											</h:selectOneMenu>
										...
										<td align="right" width="15%">
											<h:graphicImage value="images/icon_required.gif"/>
											<rich:spacer width="5"></rich:spacer>
											<h:outputText value="#{jspMsg['label.contractDocType']} New" styleClass="ms7" />
										</td>
										<td width="35%">
											<h:selectOneMenu id="ddlContractDocTypeNew" value="#{semeco001Bean.contractDocTypeNew}">
												<f:selectItems value="#{semmco001tab2Bean.contractDocTypeNew}" />	
											</h:selectOneMenu>
											
											<rich:spacer width="10"></rich:spacer> 
											<a4j:commandButton id="btnPrintNew" styleClass="rich-button"
															   value="#{jspMsg['btn.create']}" 
															   action="#{navAction.navi}"
															   reRender="frmContract,frmContractError" >
												<a4j:actionparam name="navModule" value="co" />
		...
		<rich:panel id="pnlResultContractFile">
			...
			<rich:dataTable width="97%" id="dtbContractFile" cellpadding="1"
				...
				value="#{semmco001tab2Bean.contractFileSPList}"
				rows="10" rowClasses="cur"
				styleClass="dataTable">
				<a4j:support event="onRowClick"
					action="#{semmco001tab2Action.getRowIdOnClick}"
					reRender="dtbContractFile">
					<a4j:actionparam name="rowId" value="#{contractFileSP.rowId}" />
				</a4j:support>
				<rich:column
					styleClass="#{(semmco001tab2Bean.tmpRowId==contractFileSP.rowId)?'onClick':'unClick'}">
					<f:facet name="header">
				...
				<rich:column
					...
					<h:commandLink id="btnDownloadFile" 
								   value="Download" 
								   action="#{semmco001tab2Action.doDownloadContractFile}">
								...
								<rich:columnGroup>
									<rich:column colspan="2">
										<h:outputFormat value="#{msg['message.totalRecords']}">
											<f:param value="#{fn:length(semmco001tab2Bean.contractFileSPList)}"></f:param>
										</h:outputFormat>
									</rich:column>
									<rich:column colspan="4">
										<rich:datascroller immediate="true" rendered="true" align="left" for="dtbContractFile"
											maxPages="#{semmco001tab2Bean.rowPerPage}"  selectedStyleClass="selectScroll"
											stepControls="hide" fastControls="auto" boundaryControls="auto" 
											id="dstContractFile" 
											style="background-color: #cccccc;"
											page="#{semmco001tab2Bean.scrollerPage}" 
										/>
									</rich:column>
								</rich:columnGroup>
							</f:facet>
