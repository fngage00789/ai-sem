		...
		<h:panelGrid width="100%"  border="0" cellpadding="0" cellspacing="1">
				<rich:panel id="pnlDuty">
						<f:facet name="header">
							...
							<h:graphicImage id="reqAmt" value="images/icon_required.gif"  styleClass="ms7" style="#{semmco004Bean.contract.dutyPayStatus eq '00' ? 'display:none;':''}"/>
							<rich:spacer width="5"></rich:spacer>
							<h:outputText value="#{jspMsg['label.dutyAmt']}" styleClass="ms7"/>
                			</td>
                			<td width="25%">
                			 <h:inputText id="txtDutyAmt" value="#{semmco004Bean.contract.dutyAmt}" 
       						 ...
       						 maxlength="16" 
       						 disabled="#{not semmco004Bean.renderedUpdateHistory}"
                			...
                			<td width="25%">
		                			...
		                			<tr>
		                			<td>
		                			<h:inputText id="txtCopyDutyFlag" value="#{semmco004Bean.contract.copyDuty}"  size="5"  disabled="#{not semmco004Bean.renderedUpdateHistory}"/>
		                			</td>
		                			<td>
		                			<h:outputText value="#{jspMsg['label.copy']}" styleClass="ms7"/>
		                			</td>
		                			<td>
		                			<h:inputText id="txtCoppyDutyAmt" value="#{semmco004Bean.contract.copyDutyAmt}" 
		       						 onkeypress="return numberformat.keyPressDecimalOnly(this, event);"
		       						 onblur="return numberformat.moneyFormat(this);"
		       						 onfocus="return numberformat.setCursorPosToEnd(this);"
		       						 maxlength="16" 
							...
							<h:graphicImage id="reqDt" value="images/icon_required.gif" style="#{semmco004Bean.contract.dutyPayStatus eq '00' ? 'display:none;':''}"/>
							<rich:spacer width="5"></rich:spacer>
							...
							disabled="#{not semmco004Bean.renderedUpdateHistory}"
							value="#{semmco004Bean.contract.dutyPayDt}" 
							showWeeksBar="false" 
							inputSize="13"
                			...
                			<td width="20%"> 
                			<h:inputText id="txtOtherDutyAmt" value="#{semmco004Bean.contract.otherDutyAmt}" 
       						 onkeypress="return numberformat.keyPressDecimalOnly(this, event);"
       						 onblur="return numberformat.moneyFormat(this);"
       						 onfocus="return numberformat.setCursorPosToEnd(this);"
       						 maxlength="16" 
                			...
                			<h:selectOneRadio id="rbtDutyPayStatus" value="#{semmco004Bean.contract.dutyPayStatus}"  styleClass="ms7" rendered="true"  
                				disabled="#{not semmco004Bean.renderedUpdateHistory}" 
                				 >
                				<a4j:support event="onclick" action="#{semmco004Action.checkDutyPayStatus}" reRender="popupFrmEditDuty">
                				</a4j:support>
							...
							action="#{navAction.navi}" reRender="frmError004,dtbPolicy" 
							disabled="#{not semmco004Bean.renderedUpdateHistory}"
							oncomplete="if(#{semmco004Bean.popupClose == 'true'})#{rich:component('popupEditDuty')}.hide();">
							<a4j:actionparam name="navModule" value="co" />
							<a4j:actionparam name="navProgram" value="SEMMCO004" />
							...
							<a4j:commandButton value="#{jspMsg['btn.cancel']}" styleClass="rich-button" immediate="true" disabled="#{not semmco004Bean.renderedUpdateHistory}">
