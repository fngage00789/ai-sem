...
<f:loadBundle basename="resources.insurance.semmir005" var="jspMsg"/>
<h:panelGrid width="100%">
		...
		<h:panelGrid>
			<a4j:form id="frmError">
				<rich:messages layout="list" errorClass="ms7red" warnClass="ms7blue" infoClass="ms7green" 
						rendered="#{semmir005Bean.renderedMsgFormSearch}">
				 		<f:facet name="header">
							...
							<h:panelGroup>
								<table width="100%">
									<tr>
										...
										<h:selectOneMenu id="ddlCompany" value="#{semmir005Bean.criteria.company}" onchange="GetCompany2JS();">
											<f:selectItems value="#{semmir005Bean.companyList}"/>
										</h:selectOneMenu>
										<a4j:jsFunction name="GetCompany2JS" reRender="companyDisplay"/>
										<rich:spacer width="10"></rich:spacer>
										<h:outputText id="companyDisplay" value="#{semmir005Bean.criteria.company}" styleClass="ms28"/>
					                	</td>
				                	</tr>
									<tr>
										<td align="right" width="20%">
											...
											<h:selectOneMenu id="ddlNetworkType" value="#{semmir005Bean.criteria.networkType}">
										...
										<td align="right" width="20%">
											...
											<h:selectOneMenu id="ddlTransferType" value="#{semmir005Bean.criteria.transferType}">
									...
									<tr>
										...
										<td>
											<h:selectOneMenu id="ddlpolicyType" value="#{semmir005Bean.criteria.policyType}">
										...
										<td align="right" width="20%">
											...
											<rich:calendar id="effDateFrom" locale="th" enableManualInput="true" 
												   ...
												   value="#{semmir005Bean.criteria.effDateFrom}"
												   showWeeksBar="false"
												   inputSize="10"
											...
											<rich:calendar id="effDateTo" locale="th" enableManualInput="true" 
												   datePattern="dd/MM/yyyy" 
												   value="#{semmir005Bean.criteria.effDateTo}"
												   showWeeksBar="false"
												   inputSize="10"
												   oninputkeypress="return numberformat.keyPressFormatDateOnly(this, event);"
											   	   cellWidth="15px" cellHeight="20px"
			...
			<a4j:form id="frmResult">
				...
				<h:panelGrid style="width: 90%">
					<rich:panel id="pnlSearchResult" styleClass="sem_autoScrollbar">
						...
						<div align="center">
							<h:outputLabel style="color:red;size:20px" value="#{semmir005Bean.msgDataNotFound}" rendered="#{semmir005Bean.renderedMsgDataNotFound}" />
						</div>
						<!-- begin dataTable -->
						<rich:dataTable id="dtbMpol" cellpadding="1" cellspacing="0" border="0" 
							var="MasterPol"  value="#{semmir005Bean.resultList}" reRender="dtbMpol" 
							rows="#{semmir005Bean.rowPerPage}" rowClasses="cur" styleClass="dataTable">
							<a4j:support event="onRowClick"   action="#{semmir005Action.getRowIdOnClick}" reRender="dtbMpol">
								<a4j:actionparam name="rowId" value="#{MasterPol.rowId}" />
							</a4j:support>
							<rich:column styleClass="#{(semmir005Bean.tmpRowId==MasterPol.rowId)?'onClick':'unClick'}" >
								<f:facet name="header">
							...
							<rich:column styleClass="#{(semmir005Bean.tmpRowId==MasterPol.rowId)?'onClick':'unClick'}" 
								sortBy="#{MasterPol.networkTypeDesc}" >
								<f:facet name="header">
									<h:outputText value="#{jspMsg['column.header.networkType']}" style="width: 100px"/>
								</f:facet>
								<div align="center">
									<h:outputText value="#{MasterPol.networkTypeDesc}"/>
								</div>
							</rich:column>
							<rich:column styleClass="#{(semmir005Bean.tmpRowId==MasterPol.rowId)?'onClick':'unClick'}" 
								sortBy="#{MasterPol.company}">
								<f:facet name="header">
									<h:outputText value="#{jspMsg['column.header.company']}" style="width: 100px"/>
								</f:facet>
								<div align="center">
									<h:outputText value="#{MasterPol.company}" />
								</div>
							</rich:column>
							<rich:column styleClass="#{(semmir005Bean.tmpRowId==MasterPol.rowId)?'onClick':'unClick'}" 
								sortBy="#{MasterPol.transferTypeDesc}" >
								<f:facet name="header">
									<h:outputText value="#{jspMsg['column.header.transferType']}" style="width: 100px"/>
								</f:facet>
								<div align="center">
									<h:outputText value="#{MasterPol.transferTypeDesc}"/>
								</div>
							</rich:column>
							<rich:column styleClass="#{(semmir005Bean.tmpRowId==MasterPol.rowId)?'onClick':'unClick'}" 
								sortBy="#{MasterPol.policyType}" >
								<f:facet name="header">
									<h:outputText value="#{jspMsg['column.header.policy']}" style="width: 100px"/>
								</f:facet>
								<div align="center">
									<h:outputText value="#{MasterPol.policyTypeDesc}"></h:outputText>
								</div>
							</rich:column>
							<rich:column styleClass="#{(semmir005Bean.tmpRowId==MasterPol.rowId)?'onClick':'unClick'}" 
								sortBy="#{MasterPol.premiumRate}" >
								<f:facet name="header">
									<h:outputText value="#{jspMsg['column.header.premium']}" style="width: 100px"/>
								</f:facet>
								<div align="right">
									<h:outputText value="#{MasterPol.premiumRate}">
										<f:convertNumber pattern="#,##0.00"  maxIntegerDigits="13" maxFractionDigits="8" />
									</h:outputText>
								</div>
							</rich:column>
							<rich:column styleClass="#{(semmir005Bean.tmpRowId==MasterPol.rowId)?'onClick':'unClick'}" sortBy="#{MasterPol.effDate}">
								<f:facet name="header">
									<h:outputText value="#{jspMsg['column.header.effectiveDate']}" style="width: 100px"/>
								</f:facet>
								<div align="center">
									<h:outputText value="#{MasterPol.effDateStr}">
									</h:outputText>
								</div>
							</rich:column>
							<rich:column styleClass="#{(semmir005Bean.tmpRowId==MasterPol.rowId)?'onClick':'unClick'}"  sortBy="#{MasterPol.updateBy}">
								<f:facet name="header">
									<h:outputText value="#{jspMsg['column.header.updateBy']}" style="width: 100px"/>
								</f:facet>
								<div align="center">
									<h:outputText value="#{MasterPol.updateBy}"></h:outputText>
								</div>
							</rich:column>
							<rich:column styleClass="#{(semmir005Bean.tmpRowId==MasterPol.rowId)?'onClick':'unClick'}" sortBy="#{MasterPol.updateDt}" >
								<f:facet name="header">
									<h:outputText value="#{jspMsg['column.header.updateDate']}" style="width: 100px"/>
								</f:facet>
								<div align="center">
									<h:outputText value="#{MasterPol.updateDtStr}">
									</h:outputText>
								</div>
							</rich:column>
							<!-- end column -->
								...
								<rich:columnGroup>
									<rich:column colspan="3">
										<h:outputFormat value="#{msg['message.totalRecords']}">
											<f:param value="#{fn:length(semmir005Bean.resultList)}"></f:param>
										</h:outputFormat>
									</rich:column>
									<rich:column colspan="8">
										<rich:datascroller immediate="true" rendered="true" align="left" for="dtbMpol"
											maxPages="#{semmir005Bean.rowPerPage}"  selectedStyleClass="selectScroll"
											stepControls="hide" fastControls="auto" boundaryControls="auto" 
											style="background-color: #cccccc;" 
											page="#{semmir005Bean.scrollerPage}" 
										/>
									</rich:column>
								</rich:columnGroup>
							</f:facet>
	...
	<jsp:include page="../../pages/ir/semmir005-edit.jsp" />
</h:panelGrid>
