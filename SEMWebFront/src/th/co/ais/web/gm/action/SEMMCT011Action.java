...
import th.co.ais.web.gm.bean.SEMMCT011Bean;
...
public class SEMMCT011Action extends AbstractAction{
...
//	
	private String getVendorSelected(){
		List<WrapperBeanObject<VendorMasterSP>> vendorMasterList = getSemmct011Bean().getVendorMasterList();
		String strBuff = "";
		StringBuffer b = new StringBuffer();
		if(vendorMasterList != null && !vendorMasterList.isEmpty()){
			for (WrapperBeanObject<VendorMasterSP> wrapperBeanObject : vendorMasterList) {
				...
				if(StringUtils.equals(navPrograme, "semmct001-1")){
					...
					String vendorMasterSel = getSemmct011Bean().getVendorMaster().getRowId();
					b.append(",");
					b.append(vendorMasterSel);
					break;
				}
...
//		String taxId13 = ct001Bean.getVendorMaster().getTaxId13();
//		
//		
//		IVendorMasterService service = (IVendorMasterService)getBean("vendorMasterService");
...
//	
	private boolean validateFrmSearch(){
		...
		boolean flag = false;
		SEMMCT011Bean ct001Bean = getSemmct011Bean();
...
//			System.out.println("recordStatus :="+ct001Bean.getCriteriaVendorSP().getRecordStatus());
//			System.out.println("payeeName :="+ct001Bean.getCriteriaVendorSP().getPayeeName());
...
//			}
			vendorMasterList = service.queryVendorMasterSPList(EQueryName.SP_MCT011_SRCH.name, ct001Bean.getCriteriaVendorSP());
			if(vendorMasterList != null && !vendorMasterList.isEmpty()){
				 for(int i=0; i<vendorMasterList.size(); i++){
					...
					if(vm.getCreateDt() != null){
						vm.setCreateDtStr(SEMDataUtility.convertToThYearStr(vm.getCreateDt()));
					}
					if(vm.getApproveDT() != null){
						vm.setApproveDtStr(SEMDataUtility.convertToThYearStr(vm.getApproveDT()));
					}
					
					if(vm.getRecordStatus() != null){ 
						if(vm.getRecordStatus().equals("02") ){
//							if(vm.getRecordStatus().equals("03")){
//								semmct011Bean.setRenderedApproveBtn(false);
//								semmct011Bean.setRenderedRejectBtn(true);
//							}else if(vm.getRecordStatus().equals("04")){
//								semmct011Bean.setRenderedApproveBtn(true);
//								semmct011Bean.setRenderedRejectBtn(false);
//							}
							vm.setRenderedChkSelect(true);
						}else{
//							semmct011Bean.setRenderedApproveBtn(false);
//							semmct011Bean.setRenderedRejectBtn(false);
				 ...
				 ct001Bean.setDisabledBtnApproveAndReject(true);
			 }else{
				 ct001Bean.setRenderedMsgDataNotFound(true);
			 }
		} catch (Exception e) {
...
//			vendorSP.setTaxId13(ct001Bean.getVendorMaster().getTaxId13());
//			
...
//				
//				String expenseType = (String)getFacesUtils().getRequestParameter("expenseType");
//				getSemmct011Bean().getVendorMapPayee().setExpenseType(expenseType);
//				getSemmct011Bean().setDisableSavePayeeBtn(false);
...
//				
//				//set default payment type = 01
//				getSemmct011Bean().getVendorMapPayee().setPaymentType("01");
//				getSemmct011Bean().setDisableSavePayeeBtn(true);
//			}
////			else if(ServiceConstants.MODULE_ACTION_SELECT.equals(mode)){
////				setSelectedPayee();
////			}
//			String paymentType = getSemmct011Bean().getVendorMapPayee().getPaymentType();
//			
//			if(StringUtils.equals(paymentType, "01"))
//			getSemmct011Bean().setForceInputBankInfo(true);
//			
//			//for inform mode when submit
//			getSemmct011Bean().setMode(mode);
//			//for displaying in pop up header
//			getSemmct011Bean().setActModeDisplay(getDisplayMode(mode));
//			
//		}catch (Exception e) {
//			e.printStackTrace();
//		}
...
//		//clear all of value.
//		getSemmct011Bean().setPayeeMaster(new PayeeMaster());
//		//getSemmct011Bean().setVendorMapPayee(new VendorMapPayee());
//		getSemmct011Bean().setPayeeBookBank(new PayeeBookbank());
//		getSemmct011Bean().setCt001SrchMSP(new CT001SrchMSP());
//	}
...
//	}
	...
	@Override
	public void init() {
		setSemmct011Bean(new SEMMCT011Bean());
		
		SEMMCT011Bean semmct011Bean = getSemmct011Bean();
		semmct011Bean.setCompanyList(getLovItemsByType(ELovType.T_COMPANY.name));
		semmct011Bean.setRegionList(RegionCacheUtil.getInstance().getRegionSelList());
		semmct011Bean.setVendorStatusSelList(getLovItemsByType(ELovType.T_CT_ACCOUNT_STATUS.name));
		semmct011Bean.setBookbankStatusSelList(getLovItemsByType(ELovType.T_CT_BOOKBANK_STATUS.name));
		semmct011Bean.setVendorTypeStatus(getLovItemsByType(ELovType.T_CT_VENDOR_TYPE.name));
		semmct011Bean.setProvinceSelList(ProvinceCacheUtil.getInstance().getProvinceSelList());
		semmct011Bean.setExpenseTypeSelList(getLovItemsByType(ELovType.T_CT_EXPENSE_TYPE.name));
		semmct011Bean.setBankAccountSelList(getLovItemsByType(ELovType.T_CT_BANK_ACC_TYPE.name));
		semmct011Bean.setHqList(getLovItemsByType(ELovType.T_CT_VENDOR_BRANCH.name));
		setSemmct011Bean(semmct011Bean);
...
//		String taxId13 = getSemmct010Bean().getVendorMaster().getTaxId13();
...
//	
	public void onRender(){
		...
		getSemmct011Bean().setTmpRowId(rowId);
		
		if(isCheckSELBox()==false){
//			addMessageError("E0001");
			getSemmct011Bean().setDisabledBtnApproveAndReject(true);
		}else{
			getSemmct011Bean().setDisabledBtnApproveAndReject(false);
		}
...
//	
	private boolean isCheckSELBox(){
		...
		List<WrapperBeanObject<VendorMasterSP>> vendorMasterSP = getSemmct011Bean().getVendorMasterList();
...
//				detailList = getSemmct011Bean().getCt001SrchMSPList();
//				for(int i=0; i<detailList.size(); i++){
//					detailList.get(i).setCheckBox(chkAll);
//				}
...
//	
	public void onRenderApproveButton(){
		if(isCheckSELBoxApprove())
			getSemmct011Bean().setDisabledButtonApprove(false);
		else
			getSemmct011Bean().setDisabledButtonApprove(true);
		
		if(!getSemmct011Bean().isChkSelAll()){
			String vendorMapPayeeId = (String)getFacesUtils().getRequestParameter("vendorMapPayeeId");
			LOG.debug("vendorMapPayeeId = "+vendorMapPayeeId);
			if(vendorMapPayeeId != null){
				for(WrapperBeanObject<CT001SrchMSP> obj :getSemmct011Bean().getCt001SrchMSPList()){
	...
	private boolean isCheckSELBoxApprove(){
		boolean isCheck = false;
		List<WrapperBeanObject<CT001SrchMSP>> approveList = getSemmct011Bean().getCt001SrchMSPList();
	...
	private SEMMCT011Bean semmct011Bean;
	
	public SEMMCT011Bean getSemmct011Bean() {
		SEMMCT011Bean ct001Bean  =(SEMMCT011Bean)getFacesUtils().getSessionMapValue("semmct011Bean");
		if(ct001Bean == null)
		ct001Bean = new SEMMCT011Bean();
		return ct001Bean;
	}

	public void setSemmct011Bean(SEMMCT011Bean semmct011Bean) {
		this.semmct011Bean = semmct011Bean;
		getFacesUtils().setSessionMapValue("semmct011Bean", semmct011Bean);
	}
...
//	
	...
	public boolean doSaveAct(){
		...
		for (WrapperBeanObject<VendorMasterSP> temp : semmct011Bean.getVendorMasterList()) {
		...
		semmct011Bean.setApproveBookBankAct(new ApproveBookBankAct());
		semmct011Bean.getApproveBookBankAct().setVendorMapPayeeId(vendorMapPayeeIdConCat);
		semmct011Bean.getApproveBookBankAct().setRemark(semmct011Bean.getRemark());
		semmct011Bean.getApproveBookBankAct().setActionType(semmct011Bean.getBtnApproveStatus());
		semmct011Bean.getApproveBookBankAct().setUserId(getUserLogIn());

		System.out.println("vendorMapPayeeIdConCat =: "+vendorMapPayeeIdConCat);
		System.out.println("semmct011Bean.getRemark() =: "+semmct011Bean.getRemark());
		System.out.println("semmct011Bean.getBtnApproveStatus() =: "+semmct011Bean.getBtnApproveStatus());
		try {
			to = lovMasterService.querySPList(EQueryName.SP_APPROVE_BOOK_BANK_ACT.name, semmct011Bean.getApproveBookBankAct());
			if (to != null && !to.isEmpty() && to.get(0).getResultMsg().equals("Success")) {
				addMessageInfo("M0001");
				semmct011Bean.setPopupClose(true);
				doSearch();
				doClearApproveStatus();
				semmct011Bean.setDisabledBtnApproveAndReject(true);
			}else if(to != null && !to.isEmpty()){
				doClearApproveStatus();
				semmct011Bean.setRenderedMsgFormSearch(true);
				FrontMessageUtils.addMessageError(to.get(0).getpRemark());
			}
			semmct011Bean.setPopupClose(true);
			setSemmct011Bean(semmct011Bean);
...
//		getSemmct011Bean().setRenderedMsgFormSearch(false);
//		getSemmct011Bean().setRenderedMsgFormMiddle(true);
		return flag;
	}
	...
	public boolean doClearApproveStatus(){
		boolean flag = false;
		semmct011Bean = getSemmct011Bean();
		semmct011Bean.setBtnApproveStatus("");
		semmct011Bean.setRemark("");
		setSemmct011Bean(semmct011Bean);
		return flag;
	}
}
