...
public class SEMMRT009Action extends AbstractAction{
...
//		semmrt003Bean.setRentalPayExpenseTypeList(LOVCacheUtil.getInstance().getRentalPayExpenseTypeSelList());
	...
	public boolean doSearch(){
		...
		// incase search from To Do List
			...
			if(semmrt003Bean.getTmpParamStr() != null){
				semmrt003Bean.getRentalPayNormalSearchSP().setStrParam(semmrt003Bean.getTmpParamStr());
			}
		}
...
//		log.debug("");
		List<RentalPayNormalSearchSP> to = null;
			try {
				to = rentalPaymentService.querySPList(EQueryName.Q_SEARCH_RENTALPAY_NORMAL_LP.name, semmrt003Bean.getRentalPayNormalSearchSP());
...
//							}
						 if(rentalPayNormalSearchSP != null){
...
//									rentalPayNormalSearchSP.setTransferDtDisplay(SEMDataUtility.convertToThYear(rentalPayNormalSearchSP.getTransferDt()));
				...
				semmrt003Bean.setRenderedMsgFormTop(true);
				semmrt003Bean.setRenderedMsgFormMiddle(false);
				semmrt003Bean.setTmpParamStr(semmrt003Bean.getRentalPayNormalSearchSP().getStrParam());
				semmrt003Bean.getRentalPayNormalSearchSP().setStrParam("");
				setSemmrt003Bean(semmrt003Bean);
				doSearcRentalPayS();
				flag = true;
...
//				}
			...
			try {
				to = rentalPaymentService.querySPList(EQueryName.SP_MRT009_ACT.name, semmrt003Bean.getRentalPayNormalSaveActSP());
				if (to != null && !to.isEmpty() && to.get(0).getResultMsg().equals("Success")) {
					addMessageInfo("M0001");
				}else if(to != null && !to.isEmpty()){
					FrontMessageUtils.addMessageError(to.get(0).getpRemark());
...
//				semmrt003Bean.getRentalPayNormalSearchDSP().setPaymentMethod("03");
