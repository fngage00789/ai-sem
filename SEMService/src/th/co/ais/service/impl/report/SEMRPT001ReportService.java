...
public class SEMRPT001ReportService extends ReportService {
	private static Logger log =Logger.getLogger(SEMRPT001ReportService.class);
	...
	public JasperPrint execute(ReportParameter param) throws ReportServiceException {
		...
		try {
			...
			jasperPrint = JasperFillManager.fillReport(jasperReport, ((SEMRPT001ReportParameter)param).getReportParameter(), conn);
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
	...
	public JasperPrint execute(String paramXML) throws ReportServiceException {
		...
		try {
			if(paramXML!=null){
				...
				param = (SEMRPT001ReportParameter)jibxUnMarshaller.unmarshal(ss);
				if(param!=null){
	...
	public String getReportName(){
		...
		reportName = "SEMRPT001" + "_" + new SimpleDateFormat("yyyyMMddHHmmss").format(new Date()) + "_" + new Random().nextInt(10000);
		return reportName;
	}
