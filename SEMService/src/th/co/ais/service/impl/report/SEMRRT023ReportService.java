...
public class SEMRRT023ReportService extends ReportService {
	private static Logger log =Logger.getLogger(SEMRRT023ReportService.class);
	...
	public JasperPrint execute(ReportParameter param) throws ReportServiceException {
		...
		try {
			...
			jasperPrint = JasperFillManager.fillReport(jasperReport, ((SEMRRT023ReportParameter)param).getReportParameter(), conn);
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
	...
	public JasperPrint execute(String paramXML) throws ReportServiceException {
		...
		try {
			if(paramXML!=null){
				...
				param = (SEMRRT023ReportParameter)jibxUnMarshaller.unmarshal(ss);
				if(param!=null){
	...
	public String getReportName(){
		...
		reportName = "SEMRRT023" + "_" + new SimpleDateFormat("yyyyMMddHHmmss").format(new Date()) + "_" + new Random().nextInt(10000);
		return reportName;
	}
