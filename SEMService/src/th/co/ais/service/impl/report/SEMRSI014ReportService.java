...
public class SEMRSI014ReportService extends ReportService {

	private static Logger log = Logger.getLogger(SEMRSI005ReportService.class);
	...
	public JasperPrint execute(ReportParameter param)
		...
		try {
			...
			jasperPrint = JasperFillManager.fillReport(jasperReport,
					((SEMRSI014ReportParameter) param).getReportParameter(),
					conn);
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
	...
	public JasperPrint execute(String paramXML) throws ReportServiceException {
		...
		SEMRSI014ReportParameter param = null;
		try {
			if (paramXML != null) {
				sr = new StringReader(paramXML);
				ss = new StreamSource(sr);
				param = (SEMRSI014ReportParameter) jibxUnMarshaller
						.unmarshal(ss);
				if (param != null) {
