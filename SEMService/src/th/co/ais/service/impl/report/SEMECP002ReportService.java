...
import th.co.ais.dao.rpt.jdbc.SEMECP002ReportDao;
 ...
 * 
 */
public class SEMECP002ReportService extends ReportService<SEMECP002Domain> {
	private static Logger log =Logger.getLogger(SEMECP002ReportService.class);

	private SEMECP002ReportDao semecp002ReportDao;
	
	public void setSemecp002ReportDao(SEMECP002ReportDao semecp002ReportDao) {
		this.semecp002ReportDao = semecp002ReportDao;
	}
	...
	public JasperPrint execute(ReportParameter param) throws ReportServiceException {
		...
		try {
			...
			jasperPrint = JasperFillManager.fillReport(jasperReport, ((SEMECP002ReportParameter)param).getReportParameter(), conn);
		} catch (Exception e) {
			log.error(e);
	...
	public JasperPrint execute(String paramXML) throws ReportServiceException {
		...
		try {
			param = (SEMECP002ReportParameter)genReportParameter(paramXML);
			if(param!=null){
	...
	public String getReportName(){
		...
		reportName = "SEMECP002" + "_" + new SimpleDateFormat("yyyyMMddHHmmss").format(new Date()) + "_" + new Random().nextInt(10000);
		return reportName;
	}
	...
	public String export(String paramXML, TemplateIdentifier templateIdentifier, String reportName) throws Exception {
		...
		FileOutputStream outputSteam = null;
		SEMECP002ReportParameter param = null;
		SEMECP002Domain domain = null;
		
		try {
			param = (SEMECP002ReportParameter)genReportParameter(paramXML);
			if (param!=null) {
				domain = (SEMECP002Domain) semecp002ReportDao.execute(param.getReportParameter());
	...
	public ReportParameter genReportParameter(String paramXML) throws ReportServiceException {
		...
		try {
			if(paramXML!=null){
				...
				param = (SEMECP002ReportParameter)jibxUnMarshaller.unmarshal(ss);
			}			
		} catch (Exception e) {
			if(e instanceof ReportServiceException){
				throw (ReportServiceException)e;
