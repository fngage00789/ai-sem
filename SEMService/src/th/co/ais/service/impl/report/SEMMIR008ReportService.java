...
import th.co.ais.dao.rpt.jdbc.SEMMIR008ReportDao;
import th.co.ais.dao.rpt.jdbc.SEMMRT003ReportDao;
 ...
 * 
 */
public class SEMMIR008ReportService extends ReportService<SEMMIR008Domain> {
	private static Logger log =Logger.getLogger(SEMMIR008ReportService.class);
	
	private SEMMIR008ReportDao semmir008ReportDao;
	
	public void setSemmir008ReportDao(SEMMIR008ReportDao semmir008ReportDao) {
		this.semmir008ReportDao = semmir008ReportDao;
	}
	...
	public JasperPrint execute(ReportParameter param) throws ReportServiceException {
		...
		try {
			...
			jasperPrint = JasperFillManager.fillReport(jasperReport, ((SEMMRT003ReportParameter)param).getReportParameter(), conn);
		} catch (Exception e) {
			log.error(e);
	...
	public JasperPrint execute(String paramXML) throws ReportServiceException {
		...
		try {
			param = (SEMMRT003ReportParameter)genReportParameter(paramXML);
			if(param!=null){
...
//		reportName = "SEMMRT003" + "_" + new SimpleDateFormat("yyyyMMddHHmmss").format(new Date()) + "_" + new Random().nextInt(10000);
		reportName = "SEMMIR008" + new SimpleDateFormat("yyyyMMddHHmmss").format(new Date());
		return reportName;
	}
	...
	public String export(String paramXML, TemplateIdentifier templateIdentifier, String reportName) throws Exception {
		...
		List<SEMMIR008Domain> voList = null;
		ResultSet rs = null;
		
		try {
			param = (SEMMIR008ReportParameter)genReportParameter(paramXML);
			if (param!=null) {
				voList = semmir008ReportDao.executeList(param.getReportParameter());
	...
	public ReportParameter genReportParameter(String paramXML) throws ReportServiceException {
		...
		try {
			if(paramXML!=null){
				...
				param = (SEMMIR008ReportParameter)jibxUnMarshaller.unmarshal(ss);
			}			
		} catch (Exception e) {
			if(e instanceof ReportServiceException){
				throw (ReportServiceException)e;
