...
public class SEMREL004ReportService extends ReportService {
	private static Logger log =Logger.getLogger(SEMREL004ReportService.class);
	...
	public JasperPrint execute(ReportParameter param) throws ReportServiceException {
		...
		try {
			...
			jasperPrint = JasperFillManager.fillReport(jasperReport, ((SEMREL004ReportParameter)param).getReportParameter(), conn);
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
	...
	public JasperPrint execute(String paramXML) throws ReportServiceException {
		...
		try {
			if(paramXML!=null){
				...
				param = (SEMREL004ReportParameter)jibxUnMarshaller.unmarshal(ss);
				if(param!=null){
