...
public class SEMRPT002ReportService extends ReportService {
	private static Logger log =Logger.getLogger(SEMRPT002ReportService.class);
	...
	public JasperPrint execute(ReportParameter param) throws ReportServiceException {
		...
		try {
			...
			jasperPrint = JasperFillManager.fillReport(jasperReport, ((SEMRPT002ReportParameter)param).getReportParameter(), conn);
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
	...
	public JasperPrint execute(String paramXML) throws ReportServiceException {
		...
		try {
			if(paramXML!=null){
				...
				param = (SEMRPT002ReportParameter)jibxUnMarshaller.unmarshal(ss);
				if(param!=null){
	...
	public String getReportName(){
		...
		reportName = "SEMRPT002" + "_" + new SimpleDateFormat("yyyyMMddHHmmss").format(new Date()) + "_" + new Random().nextInt(10000);
		return reportName;
	}
