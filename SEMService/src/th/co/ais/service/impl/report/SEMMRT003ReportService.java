 ...
 * 
 */
public class SEMMRT003ReportService extends ReportService<SEMMRT003Domain> {
	private static Logger log =Logger.getLogger(SEMMRT003ReportService.class);
	
	private SEMMRT003ReportDao semmrt003ReportDao;
	
	public void setSemmrt003ReportDao(SEMMRT003ReportDao semmrt003ReportDao) {
		this.semmrt003ReportDao = semmrt003ReportDao;
	}
...
//		reportName = "SEMMRT003" + "_" + new SimpleDateFormat("yyyyMMddHHmmss").format(new Date()) + "_" + new Random().nextInt(10000);
		reportName = "RENTAL" + new SimpleDateFormat("yyyyMMddHHmmss").format(new Date());
		return reportName;
	}
	...
	public String export(String paramXML, TemplateIdentifier templateIdentifier, String reportName) throws Exception {
		...
		FileOutputStream outputSteam = null;
		SEMMRT003ReportParameter param = null;
		List<SEMMRT003Domain> voList = null;
		ResultSet rs = null;
		
		try {
			param = (SEMMRT003ReportParameter)genReportParameter(paramXML);
			if (param!=null) {
				voList = semmrt003ReportDao.executeList(param.getReportParameter());
	...
	public ReportParameter genReportParameter(String paramXML) throws ReportServiceException {
		...
		try {
			if(paramXML!=null){
				...
				param = (SEMMRT003ReportParameter)jibxUnMarshaller.unmarshal(ss);
			}			
		} catch (Exception e) {
			if(e instanceof ReportServiceException){
				throw (ReportServiceException)e;
