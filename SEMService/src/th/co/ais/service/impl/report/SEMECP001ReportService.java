...
import th.co.ais.dao.rpt.jdbc.SEMECP001ReportDao;
 ...
 * 
 */
public class SEMECP001ReportService extends ReportService<SEMECP001Domain> {
	private static Logger log =Logger.getLogger(SEMECP001ReportService.class);
	
	private SEMECP001ReportDao semecp001ReportDao;
	
	public void setSemecp001ReportDao(SEMECP001ReportDao semecp001ReportDao) {
		this.semecp001ReportDao = semecp001ReportDao;
	}
	...
	public JasperPrint execute(ReportParameter param) throws ReportServiceException {
		...
		try {
			...
			jasperPrint = JasperFillManager.fillReport(jasperReport, ((SEMECP001ReportParameter)param).getReportParameter(), conn);
		} catch (Exception e) {
			log.error(e);
	...
	public JasperPrint execute(String paramXML) throws ReportServiceException {
		...
		try {
			param = (SEMECP001ReportParameter)genReportParameter(paramXML);
			if(param!=null){
	...
	public String getReportName(){
		...
		reportName = "SEMECP001" + "_" + new SimpleDateFormat("yyyyMMddHHmmss").format(new Date()) + "_" + new Random().nextInt(10000);
		return reportName;
	}
	...
	public String export(String paramXML, TemplateIdentifier templateIdentifier, String reportName) throws Exception {
		...
		FileOutputStream outputSteam = null;
		SEMECP001ReportParameter param = null;
		SEMECP001Domain domain = null;
		
		try {
			param = (SEMECP001ReportParameter)genReportParameter(paramXML);
			if (param!=null) {
				domain = (SEMECP001Domain) semecp001ReportDao.execute(param.getReportParameter());
	...
	public ReportParameter genReportParameter(String paramXML) throws ReportServiceException {
		...
		try {
			if(paramXML!=null){
				...
				param = (SEMECP001ReportParameter)jibxUnMarshaller.unmarshal(ss);
			}			
		} catch (Exception e) {
			if(e instanceof ReportServiceException){
				throw (ReportServiceException)e;
