...
import th.co.ais.dao.rpt.jdbc.SEMMEL009ReportDao;
import th.co.ais.rpt.domain.SEMECO001Domain;
import th.co.ais.rpt.domain.SEMMEL009Domain;
import th.co.ais.rpt.exception.ReportServiceException;
import th.co.ais.rpt.parameter.ReportParameter;
import th.co.ais.rpt.parameter.SEMMEL009ReportParameter;
...
public class SEMMEL009ReportService extends ReportService<SEMMEL009Domain> {
	private static Logger log =Logger.getLogger(SEMMEL009ReportService.class);
	private SEMMEL009ReportDao semmel009ReportDao;
	
	public void setSemmel009ReportDao(SEMMEL009ReportDao semmel009ReportDao) {
		this.semmel009ReportDao = semmel009ReportDao;
	}
	...
	public JasperPrint execute(ReportParameter param) throws ReportServiceException {
		...
		try {
			...
			jasperPrint = JasperFillManager.fillReport(jasperReport, ((SEMMEL009ReportParameter)param).getReportParameter(), conn);
		} catch (Exception e) {
			log.error(e);
	...
	public JasperPrint execute(String paramXML) throws ReportServiceException {
		JasperPrint jasperPrint = null;
		SEMMEL009ReportParameter param = null;
		try {
			param = (SEMMEL009ReportParameter)genReportParameter(paramXML);
			if(param!=null){
	...
	public String export(String paramXML, TemplateIdentifier templateIdentifier, String reportName) throws Exception {
		...
		FileOutputStream outputSteam = null;
		SEMMEL009ReportParameter param = null;
		List<SEMMEL009Domain> voList = null;
		ResultSet rs = null;
		
		try {
			param = (SEMMEL009ReportParameter)genReportParameter(paramXML);
			if (param!=null) {
				voList = semmel009ReportDao.executeList(param.getReportParameter());
	...
	public ReportParameter genReportParameter(String paramXML) throws ReportServiceException {
		...
		try {
			if(paramXML!=null){
				...
				param = (SEMMEL009ReportParameter)jibxUnMarshaller.unmarshal(ss);
			}			
		} catch (Exception e) {
			if(e instanceof ReportServiceException){
				throw (ReportServiceException)e;
