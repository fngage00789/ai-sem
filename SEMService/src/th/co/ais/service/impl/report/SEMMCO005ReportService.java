...
import th.co.ais.dao.rpt.jdbc.SEMMCO005ReportDao;
 ...
 * 
 */
public class SEMMCO005ReportService extends ReportService<SEMMCO005Domain> {
	private static Logger log =Logger.getLogger(SEMMCO005ReportService.class);
	
	private SEMMCO005ReportDao semmco005ReportDao;
	
	public void setSemmco005ReportDao(SEMMCO005ReportDao semmco005ReportDao) {
		this.semmco005ReportDao = semmco005ReportDao;
	}
	...
	public JasperPrint execute(ReportParameter param) throws ReportServiceException {
		...
		try {
			...
			jasperPrint = JasperFillManager.fillReport(jasperReport, ((SEMMCO005ReportParameter)param).getReportParameter(), conn);
		} catch (Exception e) {
			log.error(e);
	...
	public JasperPrint execute(String paramXML) throws ReportServiceException {
		...
		try {
			param = (SEMMCO005ReportParameter)genReportParameter(paramXML);
			if(param!=null){
	...
	public String getReportName(){
		...
		reportName = "SEMMCO005" + "_" + new SimpleDateFormat("yyyyMMddHHmmss").format(new Date()) + "_" + new Random().nextInt(10000);
		return reportName;
	}
	...
	public String export(String paramXML, TemplateIdentifier templateIdentifier, String reportName) throws Exception {
		...
		try {
			param = (SEMMCO005ReportParameter)genReportParameter(paramXML);
			if (param!=null) {
				rs = semmco005ReportDao.executeRS(param.getReportParameter());
				...
				grouping = new DataProviderSQLGrouping[1];
				grouping[0] = new DataProviderSQLGrouping("obj", 
						new Discriminator("CONTRACT_ID"), 
						new String[]{"CONTRACT_NO", "RENTAL_TYPE", "RENTAL_TYPE_NAME", 
							"DOC_1_1", "DOC_1_2", "DOC_2_1", "DOC_2_2", "DOC_3_1", "DOC_3_2", 
							"DOC_4_1", "DOC_4_2", "DOC_5_1", "DOC_5_2", "DOC_6_1", "DOC_6_2", 
							"DOC_7_1", "DOC_7_2", "DOC_OTHER_1", "DOC_OTHER_2", "DOC_OTHER_REMARK", "RENTAL_TYPE_OTHER_REMARK"});
				
				providerBuilder = new DataProviderBuilder();
				providerBuilder.addAll(param.getReportParameter());
				providerBuilder.addSQL(rs, grouping);
	...
	public ReportParameter genReportParameter(String paramXML) throws ReportServiceException {
		...
		try {
			if(paramXML!=null){
				...
				param = (SEMMCO005ReportParameter)jibxUnMarshaller.unmarshal(ss);
			}			
		} catch (Exception e) {
			if(e instanceof ReportServiceException){
				throw (ReportServiceException)e;
