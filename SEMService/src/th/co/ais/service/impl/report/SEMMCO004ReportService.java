...
import th.co.ais.dao.rpt.jdbc.SEMMCO004ReportDao;
 ...
 * 
 */
public class SEMMCO004ReportService extends ReportService<SEMMCO004Domain> {
	private static Logger log =Logger.getLogger(SEMMCO004ReportService.class);
	
	private SEMMCO004ReportDao semmco004ReportDao;
	
	public void setSemmco004ReportDao(SEMMCO004ReportDao semmco004ReportDao) {
		this.semmco004ReportDao = semmco004ReportDao;
	}
	...
	public JasperPrint execute(ReportParameter param) throws ReportServiceException {
		...
		try {
			...
			jasperPrint = JasperFillManager.fillReport(jasperReport, ((SEMMCO004ReportParameter)param).getReportParameter(), conn);
		} catch (Exception e) {
			log.error(e);
	...
	public JasperPrint execute(String paramXML) throws ReportServiceException {
		JasperPrint jasperPrint = null;
		SEMMCO004ReportParameter param = null;
		try {
			param = (SEMMCO004ReportParameter)genReportParameter(paramXML);
			if(param!=null){
...
//		reportName = "SEMMRT003" + "_" + new SimpleDateFormat("yyyyMMddHHmmss").format(new Date()) + "_" + new Random().nextInt(10000);
		reportName = "SEMMCO004" + new SimpleDateFormat("yyyyMMddHHmmss").format(new Date());
		return reportName;
	}
	...
	public String export(String paramXML, TemplateIdentifier templateIdentifier, String reportName) throws Exception {
		...
		FileOutputStream outputSteam = null;
		SEMMCO004ReportParameter param = null;
		List<SEMMCO004Domain> voList = null;
		ResultSet rs = null;
		
		try {
			param = (SEMMCO004ReportParameter)genReportParameter(paramXML);
			if (param!=null) {
				voList = semmco004ReportDao.executeList(param.getReportParameter());
	...
	public ReportParameter genReportParameter(String paramXML) throws ReportServiceException {
		...
		try {
			if(paramXML!=null){
				...
				param = (SEMMCO004ReportParameter)jibxUnMarshaller.unmarshal(ss);
			}			
		} catch (Exception e) {
			if(e instanceof ReportServiceException){
				throw (ReportServiceException)e;
