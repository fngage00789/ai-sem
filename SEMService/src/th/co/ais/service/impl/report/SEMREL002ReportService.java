...
public class SEMREL002ReportService extends ReportService {
	private static Logger log =Logger.getLogger(SEMREL001ReportService.class);
	...
	public JasperPrint execute(ReportParameter param) throws ReportServiceException {
		...
		try {
			...
			if(conn == null){
				log.info("Test 1:"+ conn);
			}
			log.info("Test 2:"+ jasperReport.getTitle());
			log.info("Test 3:"+ param.getReportParameter());
			
			
			jasperPrint = JasperFillManager.fillReport(jasperReport, ((SEMREL002ReportParameter)param).getReportParameter(), conn);
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
	...
	public JasperPrint execute(String paramXML) throws ReportServiceException {
		...
		try {
			if(paramXML!=null){
				...
				param = (SEMREL002ReportParameter)jibxUnMarshaller.unmarshal(ss);
				if(param!=null){
	...
	public String getReportName(){
		...
		reportName = "SEMRRT001" + "_" + new SimpleDateFormat("yyyyMMddHHmmss").format(new Date()) + "_" + new Random().nextInt(10000);
		return reportName;
	}
