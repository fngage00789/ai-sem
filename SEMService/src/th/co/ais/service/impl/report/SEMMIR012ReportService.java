...
import th.co.ais.rpt.domain.SEMMIR012Domain;
 ...
 * 
 */
public class SEMMIR012ReportService extends ReportService<SEMMIR012Domain> {
	private static Logger log =Logger.getLogger(SEMMIR012ReportService.class);
	...
	public JasperPrint execute(ReportParameter param) throws ReportServiceException {
		...
		try {
			...
			jasperPrint = JasperFillManager.fillReport(jasperReport, ((SEMMIR012ReportParameter)param).getReportParameter(), conn);
		} catch (Exception e) {
			log.error(e);
	...
	public JasperPrint execute(String paramXML) throws ReportServiceException {
		...
		try {
			param = (SEMMIR012ReportParameter)genReportParameter(paramXML);
			if(param!=null){
...
//		reportName = "SEMMRT003" + "_" + new SimpleDateFormat("yyyyMMddHHmmss").format(new Date()) + "_" + new Random().nextInt(10000);
		reportName = "SEMMIR012" + new SimpleDateFormat("yyyyMMddHHmmss").format(new Date());
		return reportName;
	}
	...
	public String export(String paramXML, TemplateIdentifier templateIdentifier, String reportName) throws Exception {
		...
		FileOutputStream outputSteam = null;
		SEMMIR012ReportParameter param = null;
		List<SEMMIR012Domain> voList = null;
		ResultSet rs = null;
		
		IrClaim irClaim = null;
		List<IrClaimDetail> irClaimDetails = null;
		
		try {
			param = (SEMMIR012ReportParameter)genReportParameter(paramXML);
			if (param!=null) {
				irClaim = irClaimDao.findByPKWithChild(param.getIrClaimId());
				if (irClaim != null) {
					irClaimDetails = irClaim.getIrClaimDetails();
				}
				
				if (param.getProvinceName() != null){
				irClaim.setProvinceName(param.getProvinceName());
				}
				if (param.getTransferTypeDesc() != null){
					irClaim.setTransferTypeDesc(param.getTransferTypeDesc());
				}
				if (param.getLossTypeDesc() != null){
					irClaim.setLossTypeDesc(param.getLossTypeDesc());
				}
				if (param.getLossSubTypeDesc() != null){
					irClaim.setLossSubTypeDesc(param.getLossSubTypeDesc());
				}
				if (param.getClaimStatusDescl() != null){
					irClaim.setClaimStatusDescl(param.getClaimStatusDescl());
				}
				...
				providerBuilder.addJavaObject(irClaim, "obj");
				providerBuilder.addJavaObject(irClaimDetails, "objList");
	...
	public ReportParameter genReportParameter(String paramXML) throws ReportServiceException {
		...
		try {
			if(paramXML!=null){
				...
				param = (SEMMIR012ReportParameter)jibxUnMarshaller.unmarshal(ss);
			}			
		} catch (Exception e) {
			if(e instanceof ReportServiceException){
				throw (ReportServiceException)e;
