...
public class SEMRRT024ReportService extends ReportService {
	private static Logger log = Logger.getLogger(SEMRRT024ReportService.class);
	...
	public JasperPrint execute(ReportParameter param)
		...
		try {
			...
			jasperPrint = JasperFillManager.fillReport(jasperReport,
					((SEMRRT024ReportParameter) param).getReportParameter(),
					conn);
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
	...
	public JasperPrint execute(String paramXML) throws ReportServiceException {
		...
		try {
			if (paramXML != null) {
				...
				param = (SEMRRT024ReportParameter) jibxUnMarshaller
						.unmarshal(ss);
				if (param != null) {
	...
	public String getReportName() {
		...
		reportName = "SEMRRT024"+"_"+ new SimpleDateFormat("yyyyMMddHHmmss").format(new Date())+"_" + new Random().nextInt(10000);
		return reportName;
	}
	...
	public void export(JasperPrint jasperPrint, String destFile,
		...
		try {
			if (exportType.trim().equals(ServiceConstants.TYPE_PDF)) {
				JasperExportManager
						.exportReportToPdfFile(jasperPrint, destFile);
				...
				xlsExporter.setParameter(
						JRXlsExporterParameter.OUTPUT_FILE_NAME, destFile);
				xlsExporter.setParameter(
				...
				textExporter.setParameter(
						JRTextExporterParameter.CHARACTER_HEIGHT, new Integer(
								15));
				textExporter.setParameter(JRTextExporterParameter.PAGE_WIDTH,
						new Integer(jasperPrint.getPageWidth()));
				textExporter.setParameter(JRTextExporterParameter.PAGE_HEIGHT,
						new Integer(jasperPrint.getPageHeight()));
	...
	public ByteArrayOutputStream view(JasperPrint jasperPrint, String exportType)
		...
		try {
			...
			if (exportType.equals(ServiceConstants.TYPE_PDF)) {
				outputStream.write(JasperExportManager
				...
				textExporter.setParameter(JRExporterParameter.JASPER_PRINT,
						jasperPrint);
				textExporter.setParameter(JRExporterParameter.OUTPUT_STREAM,
						outputStream);
				textExporter
						.setParameter(JRTextExporterParameter.CHARACTER_WIDTH,
								new Integer(5));
