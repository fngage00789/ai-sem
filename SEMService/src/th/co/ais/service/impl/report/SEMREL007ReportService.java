...
public class SEMREL007ReportService extends ReportService {
	private static Logger log =Logger.getLogger(SEMREL007ReportService.class);
	...
	public JasperPrint execute(ReportParameter param) throws ReportServiceException {
		...
		try {
			...
			jasperPrint = JasperFillManager.fillReport(jasperReport, ((SEMREL007ReportParameter)param).getReportParameter(), conn);
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
	...
	public JasperPrint execute(String paramXML) throws ReportServiceException {
		...
		try {
			if(paramXML!=null){
				...
				param = (SEMREL007ReportParameter)jibxUnMarshaller.unmarshal(ss);
				if(param!=null){
	...
	public String getReportName(){
		...
		reportName = "SEMRRT007" + "_" + new SimpleDateFormat("yyyyMMddHHmmss").format(new Date()) + "_" + new Random().nextInt(10000);
		return reportName;
	}
