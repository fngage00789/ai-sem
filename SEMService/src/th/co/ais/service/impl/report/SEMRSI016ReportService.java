...
public class SEMRSI016ReportService extends ReportService {
	...
	public JasperPrint execute(ReportParameter param)
		...
		try {
			...
			jasperPrint = JasperFillManager.fillReport(jasperReport,
					((SEMRSI016ReportParameter) param).getReportParameter(),
					conn);
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
	...
	public JasperPrint execute(String paramXML) throws ReportServiceException {
		...
		SEMRSI016ReportParameter param = null;
		try {
			if (paramXML != null) {
				sr = new StringReader(paramXML);
				ss = new StreamSource(sr);
				param = (SEMRSI016ReportParameter) jibxUnMarshaller
						.unmarshal(ss);
				if (param != null) {
