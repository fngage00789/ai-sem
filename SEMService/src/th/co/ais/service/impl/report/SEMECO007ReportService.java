...
import th.co.ais.dao.rpt.jdbc.SEMECO007ReportDao;
...
public class SEMECO007ReportService extends ReportService<SEMECO001Domain> {
	private static Logger log =Logger.getLogger(SEMECO003ReportService.class);
	
	private SEMECO001ReportDao semeco001ReportDao;
	
	private SEMECO007ReportDao semeco007ReportDao;
	
	public void setSemeco001ReportDao(SEMECO001ReportDao semeco001ReportDao) {
		this.semeco001ReportDao = semeco001ReportDao;
	}
	...
	public String getReportName(){
		...
		reportName = "SEMECO003" + "_" + new SimpleDateFormat("yyyyMMddHHmmss").format(new Date()) + "_" + new Random().nextInt(10000);
		return reportName;
	}
	...
	public String export(String paramXML, TemplateIdentifier templateIdentifier, String reportName) throws Exception {
		...
		SEMECO007Domain domain = null;
		
		try {
			param = (SEMECO001ReportParameter)genReportParameter(paramXML);
			if (param!=null) {
				//domain = (SEMECO001Domain) semeco001ReportDao.execute(param.getReportParameter());
				domain = (SEMECO007Domain) semeco007ReportDao.execute(param.getReportParameter());
...
//							   "\u0E19\u0E32\u0E07".equals(prefixName.substring(0, 3)) ||
//							   "\u0E04\u0E38\u0E13".equals(prefixName.substring(0, 3)) ||
//							   "\u0E27\u0E31\u0E14".equals(prefixName.substring(0, 3)) ||
//							   "\u0E19\u0E32\u0E07\u0E2A\u0E32\u0E27".equals(prefixName.substring(0, 6)) ||
//							   "\u0E1A\u0E23\u0E34\u0E29\u0E31\u0E17".equals(prefixName.substring(0, 6)) ||
//							   "\u0E01\u0E2D\u0E07\u0E17\u0E38\u0E19".equals(prefixName.substring(0, 6)) ||
//							   "\u0E42\u0E23\u0E07\u0E40\u0E23\u0E35\u0E22\u0E19".equals(prefixName.substring(0, 8)) ||
//							   "\u0E42\u0E23\u0E07\u0E1E\u0E22\u0E32\u0E1A\u0E32\u0E25".equals(prefixName.substring(0, 9)) ||
//							   "\u0E04\u0E13\u0E30\u0E01\u0E23\u0E23\u0E21\u0E01\u0E32\u0E23".equals(prefixName.substring(0, 10)) ||
//							   "\u0E21\u0E2B\u0E32\u0E27\u0E34\u0E17\u0E22\u0E32\u0E25\u0E31\u0E22".equals(prefixName.substring(0, 11)) ||
//							   "\u0E2B\u0E49\u0E32\u0E07\u0E2B\u0E38\u0E49\u0E19\u0E2A\u0E48\u0E27\u0E19".equals(prefixName.substring(0, 12)) ||
//							   "\u0E2D\u0E07\u0E04\u0E4C\u0E01\u0E32\u0E23\u0E1A\u0E23\u0E34\u0E2B\u0E32\u0E23".equals(prefixName.substring(0, 13)) ||
//							   "\u0E19\u0E34\u0E15\u0E34\u0E1A\u0E38\u0E04\u0E04\u0E25\u0E2D\u0E32\u0E04\u0E32\u0E23\u0E0A\u0E38\u0E14".equals(prefixName.substring(0, 17)) ||
//							   "\u0E2B\u0E49\u0E32\u0E07\u0E2B\u0E38\u0E49\u0E19\u0E2A\u0E48\u0E27\u0E19\u0E2A\u0E32\u0E21\u0E31\u0E0D\u0E19\u0E34\u0E15\u0E34\u0E1A\u0E38\u0E04\u0E04\u0E25".equals(prefixName.substring(0, 26))){
//								domain.setIS_PERSON(true);
//							}else{
//								domain.setIS_PERSON(false);
//							}
				...
				// Delete postFix name from signature 'By Name'.
				try{
...
//					if(StringUtils.isNotEmpty(domain.getSIGN_NAME())){
//						String s = domain.getSIGN_NAME().split(" ")[domain.getSIGN_NAME().split(" ").length - 1];
//						if(StringUtils.isNotEmpty(domain.getOWNER_NAME())){
//							String o[] = domain.getOWNER_NAME().split(" ");
//							String t = domain.getOWNER_NAME().split(" ")[domain.getOWNER_NAME().split(" ").length - 1];
//							String lc = "";
//							if("\u0E1C\u0E39\u0E49\u0E14\u0E33\u0E40\u0E19\u0E34\u0E19\u0E01\u0E32\u0E23\u0E41\u0E17\u0E19".indexOf(s.substring(0,15)) != -1){
//								domain.setLESSOR_COMPANY(o[0]);
//								for(int i = 2 ; i< o.length;i++){
//									if(!t.equals(o[i])){
//										lc = lc+o[i]+" ";
//									}
//								}
//								domain.setSIGN_NAME(lc.trim());
//							}
//						}
//					}
//				}catch (IndexOutOfBoundsException out) {}
				...
				if(domain.getDOC_DT() != null){
					SimpleDateFormat formatter = new SimpleDateFormat("dd MMM yyyy", new Locale("th", "TH"));
					String docDt = formatter.format(domain.getDOC_DT());
					domain.setDOC_DT_STR(docDt);
					log.debug("####### " + domain.getDOC_DT_STR());
	...
	public String export(String paramXML, TemplateIdentifier templateIdentifier, String reportName, Object obj) throws Exception {
		DataProviderGrouping[] grouping = null;
		DataProvider provider = null;
		DataProviderBuilder providerBuilder = null;
		FileOutputStream outputSteam = null;
...
//				domain = (SEMECO001Domain) semeco001ReportDao.execute(param.getReportParameter());
				...
				domain = (SEMECO007Domain)obj;
				
				//edit on 11/03/2021 get getDocDtStr from db
//				if(domain.getMasterContractTitleSP().getDocDt() != null){
//					SimpleDateFormat formatter = new SimpleDateFormat("dd MMMM yyyy", new Locale("th", "TH"));
//					String docDt = formatter.format(domain.getMasterContractTitleSP().getDocDt());
//					domain.getMasterContractTitleSP().setDocDtStr(docDt);
//				}
				...
				providerBuilder = new DataProviderBuilder();
				providerBuilder.addAll(param.getReportParameter());
				providerBuilder.addJavaObject(domain.getMasterContractTitleSP(), "obj");
				providerBuilder.addJavaObject(domain.getMasterContractDetailList(), "objList");
				providerBuilder.addJavaObject(domain.getMasterContractRentalList(), "objRTList");
				providerBuilder.addJavaObject(domain.getMasterContractELList(), "objELList");
				provider = providerBuilder.getDataProvider();
	...
	public SEMECO001Domain export(String paramXML, TemplateIdentifier templateIdentifier) throws Exception {
		DataProviderGrouping[] grouping = null;
		DataProvider provider = null;
		DataProviderBuilder providerBuilder = null;
		FileOutputStream outputSteam = null;
...
//				DocumentProcessor.renderDoc(templateIdentifier, provider, instruction, new FileOutputStream(new File(reportName)));
	...
	public SEMECO001Domain export(String paramXML) throws Exception {
		DataProviderGrouping[] grouping = null;
		DataProvider provider = null;
		DataProviderBuilder providerBuilder = null;
		FileOutputStream outputSteam = null;
