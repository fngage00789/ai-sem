...
import th.co.ais.dao.rpt.jdbc.SEMMCO001ReportDao;
 ...
 * 
 */
public class SEMMCO001ReportService extends ReportService<SEMMCO001Domain> {
	private static Logger log =Logger.getLogger(SEMMCO001ReportService.class);
	
	private SEMMCO001ReportDao semmco001ReportDao;
	
	public void setSemmco001ReportDao(SEMMCO001ReportDao semmco001ReportDao) {
		this.semmco001ReportDao = semmco001ReportDao;
	}
	...
	public JasperPrint execute(ReportParameter param) throws ReportServiceException {
		...
		try {
			...
			jasperPrint = JasperFillManager.fillReport(jasperReport, ((SEMMCO001ReportParameter)param).getReportParameter(), conn);
		} catch (Exception e) {
			log.error(e);
	...
	public JasperPrint execute(String paramXML) throws ReportServiceException {
		...
		try {
			param = (SEMMCO001ReportParameter)genReportParameter(paramXML);
			if(param!=null){
	...
	public String getReportName(){
		...
		reportName = "SEMMCO001" + "_" + new SimpleDateFormat("yyyyMMddHHmmss").format(new Date()) + "_" + new Random().nextInt(10000);
		return reportName;
	}
	...
	public String export(String paramXML, TemplateIdentifier templateIdentifier, String reportName) throws Exception {
		...
		try {
			param = (SEMMCO001ReportParameter)genReportParameter(paramXML);
			if (param!=null) {
				rs = semmco001ReportDao.executeRS(param.getReportParameter());
	...
	public ReportParameter genReportParameter(String paramXML) throws ReportServiceException {
		...
		try {
			if(paramXML!=null){
				...
				param = (SEMMCO001ReportParameter)jibxUnMarshaller.unmarshal(ss);
			}			
		} catch (Exception e) {
			if(e instanceof ReportServiceException){
				throw (ReportServiceException)e;
