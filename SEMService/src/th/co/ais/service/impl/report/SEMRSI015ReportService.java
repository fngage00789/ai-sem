...
public class SEMRSI015ReportService extends ReportService {
	...
	public JasperPrint execute(ReportParameter param)
		...
		try {
			...
			jasperPrint = JasperFillManager.fillReport(jasperReport,
					((SEMRSI015ReportParameter) param).getReportParameter(),
					conn);
		} catch (Exception e) {
			log.error(e);
			e.printStackTrace();
	...
	public JasperPrint execute(String paramXML) throws ReportServiceException {
		...
		SEMRSI015ReportParameter param = null;
		try {
			if (paramXML != null) {
				sr = new StringReader(paramXML);
				ss = new StreamSource(sr);
				param = (SEMRSI015ReportParameter) jibxUnMarshaller
						.unmarshal(ss);
				if (param != null) {
