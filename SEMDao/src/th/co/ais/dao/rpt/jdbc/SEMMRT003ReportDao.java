...
public class SEMMRT003ReportDao extends AbstractHibernateDAO<SEMMRT003Domain>{
	...
	public ResultSet executeRS(Map<String, Object> criteria) throws Exception {
		...
		try{
			...
			cstmt = connection.prepareCall("call SEM_SP_MRT003_PRINT(?, ?)");
	...
	public List<SEMMRT003Domain> executeList(Map<String, Object> criteria) throws Exception {
		ResultSet rs = this.executeRS(criteria);
		SEMMRT003Domain vo = null;
		List<SEMMRT003Domain> voList = new ArrayList<SEMMRT003Domain>();		
		try{
			if (rs != null) {
				while (rs.next()) {
					vo = new SEMMRT003Domain();	
