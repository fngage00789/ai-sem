...
public class SFTPUtilities {
  ...
  public Vector getFileNameAll(String path, String formatFileName) {
    ...
    System.out.println("-------------------------");
    Vector<String> resultV = new Vector();
    Vector<String> vSyn = new Vector();
    Vector<String> vDat = new Vector();
    int orgmax = 0;
    try {
      System.out.println("ls -l " + path);
      Vector v = this.channelSftp.ls(path);
        ...
        for (i = 0; i < v.size(); i++) {
          ...
          if (obj instanceof ChannelSftp.LsEntry) {
            ...
            if (!lsEntry.getFilename().equals(".") && !lsEntry.getFilename().equals("..") && 
              lsEntry.getFilename().indexOf(formatFileName) != -1)
              if (lsEntry.getFilename().indexOf(".syn") != -1) {
                System.out.println("file syn = " + lsEntry.getFilename());
                vSyn.addElement(lsEntry.getFilename().trim());
              } else if (lsEntry.getFilename().indexOf(".dat") != -1) {
                System.out.println("file dat = " + lsEntry.getFilename());
                vDat.addElement(lsEntry.getFilename().trim());
              }  
          } 
        } 
        for (i = 0; vDat != null && i < vDat.size(); i++) {
          if (vSyn.contains(Utilities.cutExtension(vDat.get(i).toString()).concat(".syn"))) {
            System.out.println("Used Filename = " + vDat.get(i).toString());
            resultV.addElement(vDat.get(i).toString().trim());
          } 
        } 
      } 
    } catch (Exception e) {
  ...
  public Integer subStringToInteger(String fileName, int lengthNum) {
    int index = fileName.indexOf(".") - lengthNum;
    int last = fileName.indexOf(".");
    Integer num = Integer.valueOf(fileName.substring(index, last));
    return num;
  }
}
