 ...
 */
public class CustomCommonsHTTPSender extends BasicHandler {

	/** Field log           */
	protected static Log log = LogFactory.getLog(CustomCommonsHTTPSender.class.getName());

	protected HttpConnectionManager connectionManager;
	protected CommonsHTTPClientProperties clientProperties;
	boolean httpChunkStream = true; //Use HTTP chunking or not.

	public CustomCommonsHTTPSender() {
		initialize();
	}
	...
	public void invoke(MessageContext msgContext) throws AxisFault {
		...
		if (log.isDebugEnabled()) {
			log.debug(Messages.getMessage("enter00", "CustomCommonsHTTPSender::invoke"));
		}
		try {
			...
			boolean isHttps = StringUtils.equalsIgnoreCase(url.getProtocol(), "https");
			if (isHttps) {
				/*
				String scheme = "https";
		...
		if (log.isDebugEnabled()) {
			log.debug(Messages.getMessage("exit00", "CustomCommonsHTTPSender::invoke"));
		}
	}
